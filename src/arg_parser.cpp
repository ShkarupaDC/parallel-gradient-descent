...
po::variables_map parse_args(int argc, char* argv[])
{
    try {
        ...
        // clang-format off
        algo.add_options()
            ("input-path,i", po::value<std::string>()->required(), "path to input NPY file")
            ("target-path,t", po::value<std::string>()->required(), "path to target NPY file")
            ("eval-path,e", po::value<std::string>()->required(), "path to evaluation NPY file")
